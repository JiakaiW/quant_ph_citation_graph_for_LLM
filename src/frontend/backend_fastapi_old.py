...
# --- Tree-First Architecture Endpoints ---
...
class TreeNodeRequest(BaseModel):
    ...
    maxY: float
    maxNodes: int = 1000
    includeLevels: Optional[List[int]] = None  # For overview mode
    visibleClusters: Optional[List[int]] = None
    minDegree: int = 0
...
class ExtraEdgesResponse(BaseModel):
    ...
    nodeFlags: Dict[str, Dict[str, bool]]  # {nodeId: {enriched: bool}}
...
def generate_level_color(level: int) -> str:
    """Generate color based on topological level."""
...
async def get_nodes_with_tree_edges(
    request: TreeNodeRequest,
    _request_id: str = None,
    _timeout: float = None
) -> NodeTreeResponse:
    """
    PRIMARY TREE-FIRST ENDPOINT: Fetch nodes + tree edges atomically.
    Guarantees connectivity and eliminates sync issues.
    """
    logger.info(f"🌳 Tree loading: box [{request.minX:.2f}, {request.maxX:.2f}, {request.minY:.2f}, {request.maxY:.2f}], max={request.maxNodes}")
    start_time = time.time()
    
    try:
        # Build spatial query with optional filters
        spatial_where = f"""
            embedding_x BETWEEN {request.minX} AND {request.maxX}
            AND embedding_y BETWEEN {request.minY} AND {request.maxY}
        """
        
        # Add cluster filter
        if request.visibleClusters:
            cluster_list = ','.join(map(str, request.visibleClusters))
            spatial_where += f" AND cluster_id IN ({cluster_list})"
        
        # Add degree filter
        if request.minDegree > 0:
            spatial_where += f" AND degree >= {request.minDegree}"
        
        # Add topological level filter for overview mode
        if request.includeLevels:
            level_list = ','.join(map(str, request.includeLevels))
            spatial_where += f" AND topo_level IN ({level_list})"
        
        # 1. Fetch nodes in viewport
        nodes_query = f"""
            SELECT 
                paper_id, title, embedding_x, embedding_y, cluster_id, 
                degree, topo_level, year
            FROM {TABLE_NAME}
            WHERE {spatial_where}
            ORDER BY degree DESC
            LIMIT {request.maxNodes + 1}
        """
        
        nodes_df = await db_manager.execute_query(
            nodes_query, [], timeout=_timeout, request_id=_request_id
        )
        
        has_more = len(nodes_df) > request.maxNodes
        if has_more:
            nodes_df = nodes_df.head(request.maxNodes)
        
        logger.info(f"   📄 Found {len(nodes_df)} nodes in viewport")
        
        if len(nodes_df) == 0:
            return NodeTreeResponse(
                nodes=[], treeEdges=[], bounds={}, hasMore=False,
                stats={"nodeCount": 0, "treeEdgeCount": 0, "loadTime": time.time() - start_time}
            )
        
        # 2. Fetch tree edges connecting these nodes
        node_ids = nodes_df['paper_id'].tolist()
        node_ids_params = ['?' for _ in node_ids]
        
        # Tree edges: both source AND target must be in viewport nodes
        tree_edges_query = f"""
            SELECT src, dst, weight
            FROM tree_edges 
            WHERE src IN ({','.join(node_ids_params)}) AND dst IN ({','.join(node_ids_params)})
        """
        
        tree_edges_df = await db_manager.execute_query(
            tree_edges_query, node_ids + node_ids, timeout=_timeout, request_id=_request_id
        )
        
        logger.info(f"   🌲 Found {len(tree_edges_df)} tree edges between nodes")
        ...
        for _, node in nodes_df.iterrows():
            ...
            color = generate_cluster_color(cluster_id)
            ...
            nodes.append({
                ...
                "attributes": {
                    ...
                    "size": max(3, min(15, node['degree'] * 0.5)),  # Size based on degree
                    "color": color,
                    "community": cluster_id,
                    "degree": int(node['degree']),
                    "level": int(node['topo_level'] or 0),
                    "year": int(node['year'] or 0)
                }
            })
        
        # 4. Format tree edges
        tree_edges = []
        for _, edge in tree_edges_df.iterrows():
            tree_edges.append({
                "source": edge['src'],
                "target": edge['dst'],
                "attributes": {
        ...
        # 5. Calculate bounds
        bounds = {
            "minX": float(nodes_df['embedding_x'].min()),
            "maxX": float(nodes_df['embedding_x'].max()),
            "minY": float(nodes_df['embedding_y'].min()),
            "maxY": float(nodes_df['embedding_y'].max())
        }
        
        elapsed = time.time() - start_time
        stats = {
            "nodeCount": len(nodes),
            "treeEdgeCount": len(tree_edges),
            "loadTime": elapsed,
            "hasMore": has_more,
            "connectivity": "guaranteed"  # Key benefit!
        }
        
        logger.info(f"✅ Tree loading complete: {len(nodes)} nodes, {len(tree_edges)} tree edges in {elapsed:.3f}s")
        ...
        logger.error(f"❌ Tree loading failed: {e}")
        raise HTTPException(status_code=500, detail=f"Tree loading error: {e}")

@app.post("/api/edges/extra-for-nodes", response_model=ExtraEdgesResponse)  
@with_cancellation(timeout=8.0)
async def get_extra_edges_for_nodes(
    request: ExtraEdgesRequest,
    _request_id: str = None,
    _timeout: float = None
) -> ExtraEdgesResponse:
    """
    SECONDARY TREE-FIRST ENDPOINT: Progressive enrichment with extra edges.
    Adds density on demand without breaking the tree structure.
    """
    logger.info(f"➕ Extra edges for {len(request.nodeIds)} nodes (max={request.maxEdges})")
    start_time = time.time()
    
    if not request.nodeIds:
        return ExtraEdgesResponse(extraEdges=[], nodeFlags={})
    
    try:
        # Fetch extra edges where both endpoints are in the node set
        node_ids_params = ['?' for _ in request.nodeIds]
        ...
        extra_edges_df = await db_manager.execute_query(
            extra_edges_query, request.nodeIds + request.nodeIds, timeout=_timeout, request_id=_request_id
        )
        
        logger.info(f"   ➕ Found {len(extra_edges_df)} extra edges")
        
        # Format extra edges with lighter styling
        extra_edges = []
        for _, edge in extra_edges_df.iterrows():
            extra_edges.append({
                "source": edge['src'],
                "target": edge['dst'],
                "attributes": {
                    "type": "extra",
                    "color": "#888888",  # Lighter color than tree edges
                    "size": 0.4,         # Thinner than tree edges
                    "weight": float(edge['weight']),
                    "edgeType": edge['edge_type']
                }
            })
        
        # Mark all requested nodes as enriched
        node_flags = {}
        for node_id in request.nodeIds:
            node_flags[node_id] = {"enriched": True}
        
        elapsed = time.time() - start_time
        logger.info(f"✅ Extra edges complete: {len(extra_edges)} edges in {elapsed:.3f}s")
        ...
        logger.error(f"❌ Extra edges failed: {e}")
        raise HTTPException(status_code=500, detail=f"Extra edges error: {e}")
...
async def get_topological_overview(
    ...
    """
    OVERVIEW TREE-FIRST ENDPOINT: Get coarse structure using topological levels.
    Shows the "big picture" DAG structure for orientation.
    """
    ...
    try:
        ...
        for level in range(maxLevels):
            ...
            level_df = await db_manager.execute_query(
                level_query, [level, maxNodesPerLevel], timeout=_timeout, request_id=_request_id
            )
